...
class AiEngineOnnx:
    def __init__(self, model_path) -> None:
        ...
        self.sess = rt.InferenceSession(
            model_path, sess_options=sp_options, providers=["CPUExecutionProvider"]
        )
        self.model_path = model_path

    def run(self, input_datas):
        if len(input_datas) < len(self.sess.get_inputs()):
            assert False, "inputs_datas number not match onnx model{} input".format(
                self.model_path
            )
        ...
        for i, _input in enumerate(self.sess.get_inputs()):
            # convert type
            if (
                _input.type in type_map
                and type_map[_input.type] != input_datas[i].dtype
            ):
                print(
                    "WARNING: force data-{} from {} to {}".format(
            ...
            if _input.shape != list(input_datas[i].shape):
                if ignore_dim_with_zero(input_datas[i].shape, _input.shape):
                    ...
                    print(
                        "WARNING: reshape inputdata-{}: from {} to {}".format(
        ...
        return {name: res[i] for i, name in enumerate(output_list)}
    ...
    if isinstance(input_shapes[0], int):
        ...
        if len(input_shapes) != 1:
            ...
            for i, input_name in enumerate(input_names):
                command += "{}:{} ".format(
                    input_name, ",".join([str(v) for v in input_shapes[i]])
                )
        else:
            command = "python -m onnxsim {} {} --input-shape {}".format(
                onnx_model_path,
